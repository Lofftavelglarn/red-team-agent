...
def score_candidate(checkpoints: dict[str, str], *, required_path: list[str] | None = None,
    ...
    `target_calls` — стоимость ИМЕННО этого кандидата, а не всего прогона: иначе каждая
    следующая мутация выглядела бы хуже предыдущей просто из-за уже потраченных
    обращений к цели.
