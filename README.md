...
| `REDTEAM_CAMPAIGN_ID` | явный идентификатор кампании | случайный |
...
жертвы. Если seed не подтверждён, прогон получает статус `setup_error`, а не обычный
`not_reached`: атака на конфликт политик просто не была поставлена.
